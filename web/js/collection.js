...
function collectionStart($container, $add) {
    ...
    if (!container.hasClass('collection_init')) {
        ...
        container.after('<a class="link_collection link_collection_add" href="javascript:collectionAdd(\''+$container+'\', true);"><span>'+addslashes(container.attr('data-text_link_add'))+'</span></a>');
    ...
    if (container.length){
        ...
        if (index < parseInt(container.attr('data-max')) || parseInt(container.attr('data-max')) == 0) {
            ...
            if (index >= parseInt(container.attr('data-min'))) {
                container.children('.control-group').last().append('<a class="link_collection link_collection_suppr hidden" id="link_suppr_'+$container+'_'+i+'" href="javascript:collectionRemove(\'link_suppr_'+$container+'_'+i+'\');" title="Supprimer"><span>'+container.attr('data-text_link_suppr')+'</span></a><br class="clear"/>');

                if ($effet)
                    container.children('.control-group').last().children('.link_collection_suppr').delay(255).fadeIn(250);
                else
                    container.children('.control-group').last().children('.link_collection_suppr').removeClass('hidden');
            }
